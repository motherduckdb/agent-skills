...
const instance = await DuckDBInstance.create(
  "md:?custom_user_agent=agent-skills/2.4.0(harness-<harness>;llm-<llm>)"
);
...
- use Python 3.11 or 3.12 to reproduce this reference stack; its tested `dbt-duckdb` path did not run reliably on Python 3.14
...
- tag long-lived workloads with `custom_user_agent`; for repo use-case builds, use `agent-skills/2.4.0(harness-<harness>;llm-<llm>)`
...
PIPELINE_USER_AGENT = "agent-skills/2.4.0(harness-<harness>;llm-<llm>)"
...
- **Tag long-lived pipeline runners with `custom_user_agent`.** This makes workload attribution and cost analysis possible later. For repo use-case builds, use `agent-skills/2.4.0(harness-<harness>;llm-<llm>)`.
