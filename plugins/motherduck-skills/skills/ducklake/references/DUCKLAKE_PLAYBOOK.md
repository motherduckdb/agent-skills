...
- If the user only needs ingestion from object storage, `load-data` may be the real skill they need, not `ducklake`.

## Escalate to higher-level skills when needed

- Use `build-data-pipeline` when DuckLake is only one layer of a broader ingestion-to-serving design.
- Use `security-governance` when BYOB, ownership, or regional placement decisions are driving the storage choice.
- Use `pricing-roi` when the real question is whether the operational overhead of DuckLake is worth it.
