...
metadata:
  ...
  version: "1.0"
  layer: use-case
  surface: app_builder_and_performance
  answer_style: "workload-fit, architecture-first, concrete-patterns"
...
This is a use-case skill. It ties together `connect`, `model-data`, `query`, `load-data`, and `explore` into a production architecture.
...
- Data model designed (see `model-data` skill)
- Familiarity with DuckDB SQL (see `query` skill)
...
Use the `model-data` skill for schema design within each customer database.
...
- **Write DuckDB SQL, not PostgreSQL SQL.** Even when connecting via the PG endpoint. See `duckdb-sql` skill.
- **Pre-aggregate data for dashboard queries.** Use materialized summary tables (see `model-data` skill) to keep query latency under 1 second.
...
- `connect` -- Establish a MotherDuck connection and authenticate via PG endpoint or native API
- `model-data` -- Design per-customer schemas and denormalized analytical tables
- `query` -- Execute DuckDB SQL queries, CTEs, and performance optimization
- `explore` -- Discover databases, tables, columns, and data shares
- `load-data` -- Ingest data from files, APIs, and cloud storage into customer databases
