...
When a use-case skill emits a native DuckDB connection, watermark it with `custom_user_agent=agent-skills/2.2.0(harness-<harness>;llm-<llm>)`. If metadata values are missing, use `harness-unknown` and `llm-unknown`.
...
const userAgent = "agent-skills/2.2.0(harness-<harness>;llm-<llm>)";
...
```text
jdbc:duckdb:md:my_database?motherduck_token=<MOTHERDUCK_TOKEN>&custom_user_agent=agent-skills/2.2.0(harness-<harness>;llm-<llm>)
```
