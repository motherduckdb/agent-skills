...
metadata:
  ...
  version: "2.0"
  layer: use-case
  surface: migration_and_ingestion
  answer_style: "blocker-first, practical, warehouse-aware"
...
Start from the end -- what does the analytics team need? Design output tables first, then work backward. Create a multi-database structure to enforce stage separation:
...
- **Separate databases by stage.** Use `raw`, `staging`, and `analytics` databases. Never mix raw data with analytical output.
