...
- When the docs MCP is unavailable, verify claims against:
  - `motherduck.com/trust-and-security`
  - the current pricing page
  - current product pages for Hypertenancy, Dives, and Postgres Integration
...
- Prefer **TypeScript/Javascript** security examples for:
  - backend APIs
  - serverless functions
  - product-side token handling
- Prefer **Python** security examples for:
  - data automation
  - backend services
  - operational scripts
- In both languages, keep credentials in environment variables or a secret manager and avoid any browser-exposed or notebook-hardcoded token pattern unless the user is explicitly working in a low-risk local setup.
...
- Public pricing and trust pages state that compliance reports are available through the commercial process, and that some commercial/security features vary by plan.
...
Do not overstate beyond those anchors. If the user needs:

- a compliance report
- a signed DPA
- a HIPAA BAA
- plan-specific contractual commitments

say that these require confirmation through current Trust/Security or commercial channels.
...
- Verify the current public region availability before answering. MotherDuck's public pricing page currently lists AWS `us-east-1` and AWS `eu-central-1`.
